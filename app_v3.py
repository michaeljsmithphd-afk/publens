...
def search_pmc_fulltext(query, max_results, email, start_date, end_date):
    Entrez.email = email
...
def search_europe_pmc(query, max_results, start_date, end_date, email, include_preprints):
...
def search_google_scholar(query, api_key, max_results, start_year, end_year, validate_links=True):
...
if search_btn:
    ...
    if use_pmc:
        with st.status("Searching PMC full text...", expanded=False):
            df_pmc, s = search_pmc_fulltext(query, limit, email, start_dt, end_dt)
    ...
    if use_eu:
        with st.status("Searching Europe PMC...", expanded=False):
            df_eu = search_europe_pmc(query, limit, start_dt, end_dt, email, include_preprints)
            if not df_eu.empty:
                frames.append(df_eu)

    if use_gs and serp_key:
        with st.status("Searching Google Scholar and validating links...", expanded=False):
            df_gs, s = search_google_scholar(query, serp_key, limit, s_year, e_year, validate_links=True)
            stats.update({**stats, **s})
            if not df_gs.empty:
                frames.append(df_gs)
